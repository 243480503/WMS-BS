...
{
    public class InOutWaterByItemController : ControllerBase
    {
        ...
        [HttpGet]
        [HandlerAjaxOnly]
        public ActionResult GetGridJson(Pagination pagination, DateTime? begin, DateTime? end, string itemName, string inOutType)
        {
        ...
        [HttpGet]
        [HandlerAjaxOnly]
        public ActionResult DownFile(DateTime? begin, DateTime? end, string itemName, string inOutType)
        {
            using (var db = new RepositoryBase().BeginTrans())
            {
                Expression<Func<T_InOutDetailEntity, bool>> query = ExtLinq.True<T_InOutDetailEntity>();
                ...
                string title = "出入库按物料统计_" + (inOutType == "OutType" ? "(出库)" : "(入库)");
