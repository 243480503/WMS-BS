...
{
    public class InOutWaterByItemLotController : ControllerBase
    {
        ...
        [HttpGet]
        [HandlerAjaxOnly]
        public ActionResult GetGridJson(Pagination pagination, DateTime? begin, DateTime? end, string lot, string itemName, string inOutType)
        {
            using (var db = new RepositoryBase().BeginTrans())
            {
                ...
                var data = db.FindList<T_InOutDetailEntity>(query).GroupBy(o =>
                     ...
                     {
                         F_Id = o.Key.ItemCode + o.Key.Lot + o.Key.OverdueDate,
        ...
        [HttpGet]
        [HandlerAjaxOnly]
        public ActionResult DownFile(DateTime? begin, DateTime? end, string lot, string itemName, string inOutType)
        {
            using (var db = new RepositoryBase().BeginTrans())
            {
                Expression<Func<T_InOutDetailEntity, bool>> query = ExtLinq.True<T_InOutDetailEntity>();
                ...
                string title = "出入库按物料批次统计_" + (inOutType == "OutType" ? "(出库)" : "(入库)");
