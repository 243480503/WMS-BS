...
{
    public class InOutWaterBySuppItemController : ControllerBase
    {
        ...
        [HttpGet]
        [HandlerAjaxOnly]
        public ActionResult GetGridJson(Pagination pagination, DateTime? begin, DateTime? end, string supplierName, string itemName, string factoryName, string inOutType)
        {
            using (var db = new RepositoryBase().BeginTrans())
            {
                ...
                var data = db.FindList<T_InOutDetailEntity>(query).GroupBy(o =>
                    ...
                    {
                        F_Id = o.Key.SupplierName + o.Key.ItemCode,
        ...
        [HttpGet]
        [HandlerAjaxOnly]
        public ActionResult DownFile(DateTime? begin, DateTime? end, string supplierName, string itemName, string factoryName, string inOutType)
        {
            using (var db = new RepositoryBase().BeginTrans())
            {
                Expression<Func<T_InOutDetailEntity, bool>> query = ExtLinq.True<T_InOutDetailEntity>();
                ...
                string title = "出入库按供应商物料统计_" + (inOutType == "OutType" ? "(出库)" : "(入库)");
