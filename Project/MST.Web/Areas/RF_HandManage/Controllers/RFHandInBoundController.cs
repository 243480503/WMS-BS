...
namespace MST.Web.Areas.RF_HandManage.Controllers
{
    public class RFHandInBoundController : ControllerBase
    {
        ...
        [HttpGet]
        [HandlerAjaxOnly]
        public ActionResult GetOrderJson()
        {
            using (var db = new RepositoryBase().BeginTrans())
            {
                IList<InBoundModel> inboundModelList = new List<InBoundModel>();

                IList<T_InBoundEntity> inBoundList = inBoundApp.FindList(o => o.State == "New").OrderByDescending(o => o.InBoundCode).ToList();

                foreach (T_InBoundEntity cell in inBoundList)
                {
                    IList<T_InBoundDetailEntity> inBoundDetailList = inBoundDetailApp.FindList(o => o.F_Id == cell.F_Id).ToList();
                    InBoundModel inboundModel = cell.ToObject<InBoundModel>();
                    inboundModel.InBoundDetailList = inBoundDetailList.ToObject<IList<InBoundDetailModel>>();

                    inboundModelList.Add(inboundModel);
                }
                return Content(inboundModelList.ToJson());
            }
        }
        #endregion
        ...
        public ActionResult GetDetailJson(string stationCode)
        {
            ...
            else if (containerKindType == "Plastic")
            {
                ...
                receiveRecord.Qty = inBoundDetailEntity.Qty - inBoundDetailEntity.CurQty;
