...
{
    public class RFInBound_RackController : ControllerBase
    {
        ...
        public ActionResult SubmitReceiveRecordForm(T_ReceiveRecordEntity postReceiveRecord)
        {
            using (var db = new RepositoryBase().BeginTrans())
            {
                ...
                logObj.Path = "RFInBound_RackController.SubmitReceiveRecordForm"; //按实际情况修改
                ...
                {
                    /*************************************************/
                    //T_ReceiveRecordEntity  postReceiveRecord
                    ...
                    T_StationEntity t_StationEntity = db.FindEntity<T_StationEntity>(o => o.StationCode == FixType.Station.StationIn_BigItem.ToString()); //大件站台
                    ...
                    IList<T_ReceiveRecordEntity> receiveRecordEntityList = db.FindList<T_ReceiveRecordEntity>(
                        o => o.BarCode == postReceiveRecord.BarCode && o.InBoundDetailID == postReceiveRecord.InBoundDetailID && o.State == "NewGroup");
                    ...
                    else
                    {
                        ...
                        foreach (T_ReceiveRecordEntity cell in temp)
                        {
                            ...
                            {
                                return Error("操作失败:当前料架中的物料不允许混料", "");
                            }
                        }
                    }
                    ...
                    else
                    {
                        ...
                        foreach (T_ReceiveRecordEntity cell in temp)
                        {
                            ...
                            {
                                return Error("操作失败:当前料架中的物料不允许混批", "");
                            }
                        }
                    }
                    ...
                    else
                    {
                        ...
                        foreach (T_ReceiveRecordEntity cell in temp)
                        {
                            ...
                            {
                                return Error("操作失败:当前料架中的物料不允许不同质检状态混放", "");
                            }
                        }
                    }
                    ...
                    {
                        return Error("操作失败:料架已被其它单据占用：" + postReceiveRecord.BarCode, "");
                    }
                    ...
                    {
                        return Error("操作失败:库存中已存在相同料架：" + postReceiveRecord.BarCode, "");
                    }
                    ...
                    {
                        postReceiveRecord.ItemBarCode = postReceiveRecord.BarCode;
                    }
                    ...
                    {
                        if (inBoundDetailEntity.IsMustQtySame == "true")
                        {
                    ...
                    /// 入库容器类型与单据物料容器类型不符
                    if (containerEntity.ContainerKind == "Plastic") return Error("容器类型错误：料箱", ""); /// 料箱
                    else if (containerEntity.ContainerKind == "Box") return Error("容器类型错误：纸箱", ""); /// 纸箱
        ...
        {
            LogObj logObj = new LogObj();
            logObj.Path = "RFInBound_RackController.DeleteRecord"; //按实际情况修改
        ...
        [HandlerAjaxOnly]
        [ValidateAntiForgeryToken]
        public ActionResult GenBigItemInBoundTask(string barCode, string PointLoc)
        {
            using (var db = new RepositoryBase().BeginTrans())
            {
                LogObj logObj = new LogObj();
                logObj.Path = "RFInBound_RackController.GenBigItemInBoundTask"; //按实际情况修改
                ...
                try
                {
                    ...
                    T_StationEntity t_StationEntity = db.FindEntity<T_StationEntity>(o => o.StationCode == FixType.Station.StationIn_BigItem.ToString()); //正常情况应该前台下拉传入当前收货站台，此处站台只一个，因此省略
                    ...
                    else
                    {
                        ...
                        string erpCode = receiveRecordList[0].ERPWarehouseCode;
                        ...
                        else  //自动指定货位
                        {
                            ...
                            loc = locationApp.GetLocIn(ref errMsg, ref log, db, containerType, t_InBoundDetailEntity.StoreAreaID, false, erpCode, receiveRecordList[0].CheckState, true,null, item);
                            if (loc == null)
                            {
                                throw new Exception(errMsg);
                            }
                        ...
                        T_EquEntity firstLine = db.FindEntity<T_EquEntity>(o => o.EquType == "MecLine" && o.GroupCode == "StationIn_Normal" && o.Sort == 1);
                        taskEntity.CurEquID3D = firstLine.F_Id;
                        ...
                        //清空站台信息
                        decimal? CurQty = (t_InBoundDetailEntity.CurQty ?? 0);

                        //t_StationEntity.BarCode = "";
                        //if (t_InBoundDetailEntity.Qty <= CurQty) //当前入库明细数量已满足
                        //{
                        //    t_StationEntity.CurOrderDetailID = "";
                        //    IList<T_InBoundDetailEntity> detailList = db.FindList<T_InBoundDetailEntity>(o => o.InBoundID == inBoundID && o.StationID == t_StationEntity.F_Id && o.F_Id != inBoundDetailID && o.State != "Over");
                        //    if (detailList.Count < 1)
                        //    {
                        //        t_StationEntity.OrderType = "";
                        //        t_StationEntity.CurOrderID = "";
                        //        t_StationEntity.BarCode = "";
                        //        t_StationEntity.WaveID = "";
                        //    }
                        //}
                        //db.Update<T_StationEntity>(t_StationEntity);


                        IList<T_ReceiveRecordEntity> notAskReceiveList = db.FindList<T_ReceiveRecordEntity>(o => string.IsNullOrEmpty(o.LocationID) && o.ReceiveStaionID == t_StationEntity.F_Id && (o.State == "LockOver" || o.State == "NewGroup") && o.InBoundDetailID == t_StationEntity.CurOrderDetailID);
                        if (t_InBoundDetailEntity.CurQty >= t_InBoundDetailEntity.Qty && notAskReceiveList.Count < 1) //单据明细已完成,且没有当前明细需要入库的其它待入库容器
                        {
                            t_StationEntity.BarCode = "";
                            t_StationEntity.CurOrderDetailID = "";
                            t_StationEntity.CurOrderID = "";
                        ...
                        TaskListNo.Add(taskEntity.TaskNo);
